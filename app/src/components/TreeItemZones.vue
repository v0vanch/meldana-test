<template>
  <div class="item" :class="{ collapsed, childless }">
    <div class="parent" :class="{ device: item.isDevice,  [item.type]: item.type }">
      
      <button class="toggle-btn" @click="collapsed = !collapsed"/>
      <div class="parent-item">
        <div class="parent-wrap" @click="editElement(item)">
      <div class="icon" :class="{ [item.type]: item.type }"/>
          <div class="label" :class="{ root: item.root, company: item.type === 'company' }">
      ...
      <button class="edit-btn" @click="editElement(item)" v-if="item.isDevice"/>
      <button class="add-btn" v-if="item.type === 'folder'"/>
</div>
      <button class="remove-btn"  v-if="item.isDevice"  @click="elementRemoveToCatalog(item)"/>
      <button class="remove-btn" @click="elementRemoveToCatalog(item)" v-if="item.type === 'folder'" />
    </div>
    </div>
    <div class="children" :class="{ collapsed }" v-for="(children) in item.children" :key="children.index">
...
<script>
import TreeItemZones from './TreeItemZones'
...
.add-btn {
  ...
  height: 24px;
  @include default-background;
  background-image: url('../assets/icons/tree/add.svg');
  background-size: 16px 16px;
}
